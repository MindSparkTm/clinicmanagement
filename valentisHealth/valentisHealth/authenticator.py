...
def is_labs(request):
    return request.user.groups.filter(Q(name='Labs') | Q(name='Admin') | Q(name='Superadmin')).exists()

def is_radiology(request):
    return request.user.groups.filter(Q(name='Radiologist') | Q(name='Admin') | Q(name='Superadmin')).exists()

def is_doctor(request):
    return request.user.groups.filter(Q(name='Doctor') | Q(name='Admin') | Q(name='Superadmin')).exists()
